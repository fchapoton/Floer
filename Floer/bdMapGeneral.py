...
def hdEllipsesGen(ellx, elly):
    resx = []
    for i, ellxi in enumerate(ellx):
        if ellxi == -1:
            resx.append(-1)
        else:
            resx.append((1 + 5 * i, 1 + 4 * ellxi[0],
                         4 + 5 * i, 2 + 4 * ellxi[1]))
    resy = []
...
def hdCond2(rect, ell, to0, toPlus):
    ...
    for i in range(n):
        for dd in [0, 1]:  # 0 vertical ell
            ...
            for x in coord:
                for y in coord:
                    hdVert = hdGetPath(1 - dd, ell[dd][i][0], ell[dd][i][1],
                                       ell[dd][i][2], ell[dd][i][3],
                                       x[0], x[1], y[0], y[1], 1 - dd * 2)  # debug
                    deltap = []
                    cc = 0
                    for p in ll:
                        ssum = 0
...
def fillLl(gen1, gen2, cache, to0, toPlus, chEll, delta, diff, hori):
    ...
    ll = [0] * (lT0 + lTp)
    for i in diff:
        tmp = delta[(0, i * 3 + gen1.xShift[i],
                     gen1.perm[i] * 3 + gen1.yShift[i],
                     i * 3 + gen2.xShift[i],
                     gen2.perm[i] * 3 + gen2.yShift[i])]
        for j in tmp:
            ll[j[0]] += j[1]
    for k in hori:
        h0 = gen1.perm.index(k)
        h1 = gen2.perm.index(k)
        tmp = delta[(1, h0 * 3 + gen1.xShift[h0],
                     gen1.perm[h0] * 3 + gen1.yShift[h0],
                     h1 * 3 + gen2.xShift[h1],
                     gen2.perm[h1] * 3 + gen2.yShift[h1])]
        for j in tmp:
            ll[j[0]] += j[1]
    return ll
...
def fillFValue(genPool, cache, ell, to0, toPlus, chEll, delta):
    ...
    for i in genPool:
        for j in i:
            for gen in j:
                gen.llRefGen = fillLl(ref, gen, cache, to0, toPlus,
                                      chEll, delta, diff, hori)
                gen.llGenRef = fillLl(gen, ref, cache, to0, toPlus,
                                      chEll, delta, diff, hori)
